...
p <- ggplot(data = bestEpicFantasy, aes(log(bestEpicFantasy$total_ratings)))+
...
#It is Harry Potter book 1 (no surprise there)
bestEpicFantasy[which.max(bestEpicFantasy$total_ratings),]
...
tp <- bestEpicFantasy
rares<-summary(bestEpicFantasy$genreVoted)<70
rareGenre<-rares[rares==TRUE]%>%names()
...
pageGenrePlt<-ggplot(data = tp,aes(x = genreVoted, y = pageCounts))+geom_boxplot(fill="dodgerblue4")+
...
#############################################################################
############# CROSS Genres With Total Numbers of Reviews ###################################
# Outcome: SUCCESSFULLY reject Null Hypothesis

pageGenrePlt<-ggplot(data = tp,aes(x = genreVoted, y = total_ratings))+geom_boxplot(fill="dodgerblue4")+
  labs(title = "Genres By Total Ratings")+
  labs(x = "Genre", y = "Total Ratings")
pageGenrePlt

kruskal.test(total_ratings~genreVoted,data = tp)
tblResults<-with(tp,tapply(total_ratings,genreVoted,median))
tblResults
...
avgGenrePlt<-ggplot(data = tp,aes(x = hasAward, y = average_rating))+geom_boxplot(fill="dodgerblue4")+
...
#############################################################################
############# CROSS Award With Page Counts###################################
#Outcome: FAIL to reject Null

pageGenrePlt<-ggplot(data = tp,aes(x = hasAward, y = pageCounts))+geom_boxplot(fill="dodgerblue4")+
  labs(title = "Award/Nomination By Pages")+
  labs(x = "Award", y = "Page Length")
pageGenrePlt

kruskal.test(pageCounts~hasAward,data = tp)
...
pageGenrePlt<-ggplot(data = tp,aes(x = hasAward, y = total_ratings))+geom_boxplot(fill="dodgerblue4")+
